...
static AstExpr parse_precedence(Parser *self, Precedence precedence) {
    ...
    if (prefix_rule == NULL) {
        report_error(self, token.location, "Expected an expression, got `%.*s` instead.", span_len(token.span), token.span.start);
    }
    ...
    for (rule = get_rule(peek(self).kind);
        ...
        if (infix_rule == NULL) {
            report_error(self, tok.location, "Expected a valid operator, got `%.*s`.", span_len(tok.span), tok.span.start);
        }
...
static AstExpr parse_int_lit(Parser *self) {
    ...
    if (err) panic("Something bad has happened.");
...
static AstExpr parse_unary(Parser *self) {
    ...
    return (AstExpr) {
        ...
        .span = {op.span.start, end.span.end},
        .node = {
...
static AstExpr parse_binary(Parser *self, const AstExpr *left) {
    ...
    return (AstExpr) {
        ...
        .span = {left->span.start, end.span.end},
        .node = {
...
static AstOperator operator_from_token(const Token token) {
    switch (token.kind) {
    case TOKEN_KIND_PLUS:        return AST_OPERATOR_PLUS;
    case TOKEN_KIND_MINUS:       return AST_OPERATOR_MINUS;
    case TOKEN_KIND_STAR:        return AST_OPERATOR_STAR;
    case TOKEN_KIND_FSLASH:      return AST_OPERATOR_FSLASH;
    case TOKEN_KIND_DOUBLE_STAR: return AST_OPERATOR_STAR;
    default: unreachable();
    }
}
